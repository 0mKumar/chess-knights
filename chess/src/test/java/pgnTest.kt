...
class PGNTest{
    ...
    @Test
    fun parseLongestTest(){
        val parser = PGNParser()
        println(parser.parse(dummyPgnLongest))
    }

    @Test
    fun ambiguityTest(){
        val fen = "rnbq1rk1/ppp1ppbp/3p1np1/8/2PPP3/2N2N2/PP2BPPP/R1BQK2R b KQ - 11 6"
        val chess = Chess(fen)
        assertEquals(fen, chess.generateFullFen())
        val originalSan = "Nbd7"
        val move = MoveValidator.StandardValidator.moveFromSan(chess, originalSan, false)
        println(move)
        assert(move != null)
